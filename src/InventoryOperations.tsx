...
export default function InventoryOperations() {
  ...
  const places = <select name="warehouse_name" required aria-label="Warehouse" className="rounded border p-2"><option value="">Warehouse</option>{warehouses.map(w => <option key={w.name}>{w.name}</option>)}</select>
  return <details className="m-4 rounded border bg-white p-4" onToggle={e => { if (e.currentTarget.open) void load().catch(error => setMessage(String(error))) }}><summary className="cursor-pointer font-semibold">Purchase receipts, adjustments & movement history</summary>
    <p role="status" className="my-2 text-sm">{message}</p><fieldset disabled={busy} className="space-y-4">
      <form className="flex flex-wrap gap-2" onSubmit={e => { e.preventDefault(); const data = new FormData(e.currentTarget); void save(`/api/orders/${data.get('order')}/receive`, { warehouse_name: data.get('warehouse_name') }) }}>
        <select name="order" required aria-label="Submitted purchase order" className="rounded border p-2"><option value="">Submitted purchase order</option>{orders.map(o => <option key={o.id} value={o.id}>{o.order_number}</option>)}</select>{places}<button className="rounded border p-2">Receive full order</button>
      </form>
      ...
      }}><select name="sku" required aria-label="Adjustment SKU" className="rounded border p-2"><option value="">SKU</option>{catalog.map(c => <option key={c.sku}>{c.sku}</option>)}</select>{places}<input name="quantity" required type="number" step="1" placeholder="Signed quantity" aria-label="Signed adjustment quantity" className="w-36 rounded border p-2" /><input name="reason" required maxLength={500} placeholder="Adjustment reason" aria-label="Adjustment reason" className="rounded border p-2" /><button className="rounded border p-2">Record adjustment</button></form>
      <button className="text-sm underline" type="button" onClick={() => void load().catch(e => setMessage(String(e)))}>Refresh history</button>
      <div className="max-h-64 overflow-auto"><table className="w-full text-left text-xs"><thead><tr>{['Time', 'Warehouse', 'SKU', 'Event', 'Stock change', 'Reservation change'].map(h => <th key={h} className="p-2">{h}</th>)}</tr></thead><tbody>{movements.map(m => <tr key={m.id}><td className="p-2">{m.created_at}</td><td>{m.warehouse_name}</td><td>{m.sku}</td><td>{m.kind}</td><td>{m.quantity_delta}</td><td>{m.reservation_delta}</td></tr>)}</tbody></table>{!movements.length && <p>No inventory movements recorded yet.</p>}</div>
    </fieldset></details>
}
